...
function badRequest(res: Response, message: string, details: Record<string, unknown> = {}) {
  return res.status(400).json({ error: 'Bad Request', code: 'BAD_REQUEST', details: { message, ...details } });
}

export function validatePatient(req: Request, res: Response, next: NextFunction) {
  const { age, language, condition } = req.body || {};
  if (typeof age !== 'number') return badRequest(res, 'Patient.age must be a number');
  if (!['en', 'es'].includes(language)) return badRequest(res, "Patient.language must be 'en' or 'es'");
  if (!['hip_replacement', 'knee_replacement'].includes(condition)) return badRequest(res, "Patient.condition invalid");
  next();
}

export function validateJourney(req: Request, res: Response, next: NextFunction) {
  ...
  const ids = new Set(journey.nodes.map((n: any) => n.id));
